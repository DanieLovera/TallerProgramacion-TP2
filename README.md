...
### Conclusiones ###  
Para finalizar se debe aclarar, que este trabajo se realizó enteramente sin operaciones de copias, para ello los constructores de copia y asignacion por copia fueron suprimidos en todas las clases, las copias se sustituyen con operaciones de movimiento de recursos. Esto promueve el desarrollo de código mas eficiente evitando copias innecesarias, en casos en los que se necesiten copias se puede proponer un método éxplicito que realice este trabajo, además de la eficiencia esto evita que se introduzcan errores en el código por copias de referencias a memoria haciendo ambigua la determinación de que objeto debe liberar un recurso compartido. En clases con recursos que no pueden ser movidos o copiados igualmente se deben suprimir constructores y operaciones de asignación, por ejemplo los monitores debido a los mutex.  
Cuando se trabajan con clases que hagan uso de la referencia de otro objeto (this), se debe tener cuidado del scope en el cual se instancian estos objetos. Un error común es instanciar estos objetos en el stack y luego enviar las referencias a (this) en otros objetos, para que esto funcione se debe asegurar que el objeto creado en el stack tenga un lifetime mayor al objeto que recibió a this, ya que si este muere el objeto que recibe a this queda apuntando a memoria que fue liberada, ocasionando accesos ilegales a posiciones de memoria que ya no le pertenecen a nuestro proceso. Para clarificar se ofrecen dos ejemplo claros en donde este problema pudo ocurrir en este trabajo:  

- El funcionamiento de un **Worker** (thread) depende de this, es decir la dirección en memoria del lugar en donde fue instanciado, si el worker se instancia en el stack, automaticamente queda asociado a una direccion de memoria que esta asociada al scope de la instancia del worker, si se intenta mover el thread tambien se movera el this que internamente utiliza y este pertenece al lugar donde fue instanciado en el stack, si por ejemplo se intentara sacar de este scope a otro externo el this podría quedar apuntando al lugar equivocado. En resumen, no es recomendable mover un thread luego de ser instanciado si no se tiene conocimiento de donde vivirá el this del thread.  
- Creación del **State Pattern**, es común que este patrón guarde una referencia al objeto que manipula como atributo para poder realizar cambios de estado internamente, sin embargo esta referencia this es un problema si el objeto fue instanciado en el stack y luego se mueve, es el mismo caso que el anterior, son referencias a posiciones de memoria que tienen un lifetime limitado. Es por esta razón que el patrón fue ligeramente modificado para evitar conocer esta referencia hasta el momento exacto en donde sea necesaria (método de la clase) removiendo la posibilidad de perder la referencia a this.  

En relación las race conditions, se debe tratar de reconocer en que posiciones del código pueden ocurrir problemas cuando el acceso a recursos compartidos sea concurrentes y se debe evitar bloquear porciones muy grandes de código de forma innecesaria, es decir mantener bloqueado el acceso a un recurso solo en la porción necesaria (critical section).
